...
const zodiacSigns = [
  ...
  { sign: "Sagittarius", start: "11-22", end: "12-21", symbol: "♐", emoji: "🏹" },
  { sign: "Capricorn", start: "12-22", end: "01-19", symbol: "♑", emoji: "🐐" },
...
export const ZodiacInput = ({ data, onUpdate }: Props) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <RadioGroup value={inputMethod} onValueChange={(value: "birthdate" | "direct") => setInputMethod(value)}>
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="birthdate" className="font-normal cursor-pointer">Calculate from birthdate</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="direct" id="direct" />
            <Label htmlFor="direct" className="font-normal cursor-pointer">Select directly</Label>
          </div>
        </RadioGroup>
      </div>
      ...
      {inputMethod === "birthdate" ? (
        ...
        <div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {zodiacSigns.map(({ sign, symbol, emoji }) => (
              <button
                key={sign}
                type="button"
      ...
      {data.zodiacSign && (
        <div>
          <Label className="text-base font-semibold mb-3 block">Your Zodiac Sign</Label>
